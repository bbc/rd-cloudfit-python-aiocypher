...
all:
	...
	@echo "make lint     - Lint using flake8
	@echo "make mypy     - Type check using mypy
...
$(TOX_ACTIVATE): $(topdir)/tox.ini $(topdir)/setup.py
	tox -e $(TOXENV) --recreate --workdir $(TOXDIR) --notest

test: $(TOX_ACTIVATE)
	tox -e $(TOXENV) --workdir $(TOXDIR)
...
rpm_spec: $(topdir)/setup.py
	$(PYTHON) $(topdir)/setup.py bdist_rpm $(RPM_PARAMS) --spec-only --dist-dir=$(RPM_PREFIX)
